...
uniform int useMask1;
uniform int useMask2;

uniform int isTranslucent;
...
void main()
{
	...
	float selfillum = mask1.a;
	if(useMask1==0) selfillum = 0.0;
	diffuse = lColor * max(max(diffuseLight, 1e-6) * visibility,selfillum * 2.0);
	...
	{	
		...
		float tintbycolor = mask2.b;
		if(useMask2==0) tintbycolor = 0.0;
		
		sColor = (sColor * (1.0 - tintbycolor)) + (color.rgb * (tintbycolor));
		
		//float specExp = 20.0;
		//float specScale = 1.25;//2.5;
		
		float specExp = specExponent / 4.0;
		float specScl = specScale / 4.0 / 4.0;
		
		if(useMask2!=0) specExp = specExp * mask2.a;
		if(useMask2!=0) specScl = specScl * mask2.r;
		
		if(RdotV > 1e-6)
		{
			specular = sColor * (pow(max(1e-6, RdotV ), specExp) * specScl * visibility);
		}
	}
	...
	float metalness = 0.0;
	if(useMask1==1) metalness = mask1.b / 4.0;
	...
	if(isTranslucent>0)
	{
		alpha = alpha - cloakIntensity;
	}
	
	gl_FragColor = vec4((light * finalcolor) + specular + rimLight,alpha);
